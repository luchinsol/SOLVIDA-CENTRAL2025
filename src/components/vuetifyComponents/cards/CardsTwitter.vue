...
const almacenNombre = ref('Fátima'); // Valor inicial que será actualizado
...
function fetchDistributorData() {
  ...
  axios.get<AlmacenData[]>(apiUrl)
    .then(response => {
      // Filtrar datos para el almacen_id 3 (distinto al de CardsMedia que usa el 2)
      const almacenData = response.data.find((item: AlmacenData) => item.almacen_id === 3);
...
function fetchPedidosData() {
  loading.value = true;
  const almacenId = 3; // Valor fijo como indicaste
