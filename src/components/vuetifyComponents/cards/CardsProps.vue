...
const almacenNombre = ref('Alto Selva Alegre'); // Valor inicial que será actualizado
...
watch(() => props.selectedDate, (newValue) => {
  ...
  fetchPedidosData(); // Cargar datos de pedidos
});
...
function fetchDistributorData() {
  ...
  axios.get<AlmacenData[]>(apiUrl)
    .then(response => {
      // Filtrar datos para el almacen_id 1
      const almacenData = response.data.find((item: AlmacenData) => item.almacen_id === 1);
...
// Función para obtener los datos de pedidos para almacen_id 1
function fetchPedidosData() {
  loading.value = true;
  const almacenId = 1; // Ajustado para almacen_id 1
  
  // Usar la URL con los parámetros proporcionados y BASE_URL
