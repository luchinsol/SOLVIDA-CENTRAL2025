...
// Nueva interfaz para los datos del pedido completo
interface PedidoCompleto {
  pedido: {
    id: number;
    total: number;
    estado: string;
    fecha: string; // Aseguramos que la fecha esté definida en la interfaz
  };
  cliente: {
    nombre: string;
    apellidos: string;
...
const almacenNombre = ref('Cerro Colorado'); // Valor inicial que será actualizado
...
// Nueva función para crear URL de Google Maps
...
function fetchDistributorData() {
  ...
  axios.get<AlmacenData[]>(apiUrl)
    .then(response => {
      // Filtrar datos para el almacen_id 2 (cambiado de 3 a 2)
      const almacenData = response.data.find((item: AlmacenData) => item.almacen_id === 2);
...
// Función modificada para obtener los datos de pedidos con almacen_id 2
function fetchPedidosData() {
  loading.value = true;
  const almacenId = 2; // Cambiado de 3 a 2
  
  // Usar la URL con los parámetros proporcionados
  const apiUrl = `${BASE_URL}/apigw/v1/pedido_distribuidor_conteo_pedidos/${localSelectedDate.value}/${almacenId}`;
  ...
  axios.get(apiUrl)
    .then(response => {
      ...
      if (pedidosData.length > 0 && pedidosData[0].almacen && pedidosData[0].almacen.nombre) {
        almacenNombre.value = pedidosData[0].almacen.nombre;
      }
    })
    .catch(error => {
...
<template>
  <v-card>
    ...
    <v-dialog
      ...
      transition="dialog-transition"
      position="center"
      class="mx-auto detail-dialog-container" 
    >
    ...
    <div class="mt-4">
      <v-table height="300px" fixed-header class="orders-table">
        <thead>
          <tr>
            <th class="text-left font-weight-bold">Id</th>
            <th class="text-left font-weight-bold">Nombre</th>
            <th class="text-left font-weight-bold">Teléfono</th>
            <th class="text-left font-weight-bold">Dirección</th>
          ...
          <template v-else>
            <tr v-for="item in pedidos" :key="item.pedido?.id || 'default'">
              ...
              <td>
                ...
                <div class="maps-link">
                  <a 
                    :href="getGoogleMapsUrl(item.direccion?.latitud, item.direccion?.longitud)" 
                    target="_blank"
              ...
              <td>
                <v-chip
                  :color="getEstadoChipColor(item.pedido?.estado)"
                  text-color="white"
                  size="small"
...
/* Estilo para los chips de estado - width aumentado */
.estado-chip {
  font-weight: 600 !important;
  min-width: 120px !important;  /* Aumentado de 100px a 120px */
  width: auto !important;       /* Asegura que se expanda si es necesario */
  text-align: center !important;
  justify-content: center !important;
  padding: 0 16px !important;   /* Aumentado de 12px a 16px */
}
